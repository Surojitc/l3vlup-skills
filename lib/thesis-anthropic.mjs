...
import { LIMITS, MAX_PROPOSALS_PER_CHUNK, MODEL_ALLOWLIST } from './thesis-cost.mjs';
...
export const PROPOSAL_TOOL = Object.freeze({
  name: 'propose_claims',
  description: `Report at most ${MAX_PROPOSALS_PER_CHUNK} material investment claims this passage makes, each anchored to an exact quotation from it.`,
  strict: true,
  input_schema: {
    type: 'object',
    properties: {
      proposals: {
        type: 'array',
        // Structural, not advisory: `strict: true` makes the schema binding,
        // so a seventh proposal is not a thing the model can return.
        maxItems: MAX_PROPOSALS_PER_CHUNK,
        items: {
          type: 'object',
          properties: {
...
export function systemPrompt(taxonomy) {
  ...
  return [
    ...
    `7. Report at most ${MAX_PROPOSALS_PER_CHUNK} claims, and only material ones: a thesis, a driver of value, a catalyst, a risk, a stance, or a statement of conviction. If the passage offers more than ${MAX_PROPOSALS_PER_CHUNK}, keep the most material and leave the rest.`,
