...
// file is small, and it is the only file a reviewer has to trust with a key.
//
// Three things it deliberately does not do.
//
// It never reads the API key. `new Anthropic()` resolves credentials from the
// environment itself; this module never touches `process.env`, never logs a
// key, and never accepts one as an argument. If there is no credential the
// SDK raises its own authentication error and we translate it into a sentence
// a person can act on.
//
...
export function actualUsd(modelId, usage) {
  ...
  if (!p) return null;
  return Number((((usage.inputTokens / 1e6) * p.inputPerMTok) + ((usage.outputTokens / 1e6) * p.outputPerMTok)).toFixed(6));
}
...
export function classifyError(err, Anthropic) {
  ...
  if (A.AuthenticationError && err instanceof A.AuthenticationError) {
    return new ModelCallError('no_credential', 'the API rejected the credential. This run reads no key of its own: set ANTHROPIC_API_KEY in the shell that launches it, or sign in with `ant auth login`.');
  }
