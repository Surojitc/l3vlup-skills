...
export const CANDIDATE_TOOL = Object.freeze({
  ...
  input_schema: {
    ...
    properties: {
      candidates: {
        type: 'array',
        // No `maxItems` here, and not by preference. A strict tool schema is
        // compiled by the API against a restricted JSON-Schema subset that
        // excludes array-size constraints, so `maxItems` under `strict: true`
        // is rejected before inference: run 35501855065 failed on its first
        // call in 221ms for exactly this, having spent nothing. The cap is
        // stated in the description above, which the model reads, and enforced
        // in code afterwards by `capCandidates`, which it cannot argue with.
        items: {
          type: 'object',
          properties: {
            ...
            horizon: { type: 'string' },
            evidenceExcerpt: { type: 'string', description: 'One contiguous run of characters copied from the passage exactly as it appears, including punctuation, spacing and capitalisation. Do not join two separate sentences, do not tidy it, and do not give its position: the passage is searched for it. Copy enough of it to occur only once.' },
          },
          required: ['issuerMention', 'paraphrase', 'kind', 'stance', 'tags', 'evidenceExcerpt'],
...
export function extractionPrompt(taxonomy) {
  ...
  return [
    ...
    '',
    '1. Every claim must carry a verbatim excerpt copied from the passage: one contiguous run of characters, exactly as it appears. Do not give its position; the passage is searched for it. Copy enough of it that it occurs only once in the passage, because an excerpt that appears twice is discarded. If you cannot copy an exact run of characters that supports a claim, do not report that claim.',
...
export const SELECTION_TOOL = Object.freeze({
  ...
  input_schema: {
    ...
    properties: {
      selections: {
        type: 'array',
        // Same as CANDIDATE_TOOL: no array-size constraint survives `strict`.
        // `resolveSelections` already truncates to MAX_CLAIMS_PER_DOCUMENT, so
        // the ceiling was never the schema's to hold.
        items: {
          type: 'object',
          properties: {
...
export function anthropicModel({ modelId, taxonomy, client, promptVersion = PROMPT_VERSION, maxOutputTokens = LIMITS.maxOutputTokensPerCall }) {
  ...
  return {
    ...
    async propose(request) {
      ...
      // Deliberately uncapped here. The cap is on *verified* candidates and is
      // applied in the runner, after each proposal has been checked against
      // the source: capping the raw list would let a malformed proposal in
      // first position crowd out a good one behind it, which is a worse
      // outcome than reading two extra proposals we then throw away.
