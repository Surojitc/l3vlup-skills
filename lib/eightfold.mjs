 ...
 */
export async function paginateEightfold(fetchPage, { maxPages = MAX_PAGES, page = PAGE, stats } = {}) {
  const seen = new Map();
  let start = 0;
  // `stats.capped` is set when the page budget, not the board, ended the walk
  // (lib/board-checks.mjs): the count is then a floor, not a census.
  let finished = false;
  for (let i = 0; i < maxPages; i++) {
    ...
    if (positions.length === 0) { finished = true; break; }
    ...
    start += page;
    if (typeof data.count === 'number' && start >= data.count) { finished = true; break; }
    if (positions.length < page) { finished = true; break; }
  }
