// Where the originals and the extracted text are allowed to live.
//
// Everything the retrieval writes is either a copyrighted document or text
// taken from one, so none of it may reach git and none of it may sit on a
// disk that is not encrypted. This module is the gate: it answers where the
// archive is, and it refuses rather than guesses. Six checks, in order, each
// with its own refusal, because "it did not run" is only useful if it says
// which condition failed.
//
// The encryption probe is platform work and is injectable, so the tests can
// exercise every refusal without a disk. On macOS, where the archive really
// lives, it asks diskutil whether the volume holding the path is encrypted;
// on Linux it asks whether the mount's backing device is a dm-crypt mapping.
// A probe that cannot tell is a refusal, never a pass.
...
export function probeEncryption(root, { platform = process.platform, run = execFileSync } = {}) {
  try {
    if (platform === 'darwin') {
      const plist = run('diskutil', ['info', '-plist', root], { encoding: 'utf8', timeout: 10_000 });
      const encrypted = /<key>Encrypted<\/key>\s*<true\/>/.test(plist);
      const locked = /<key>FilesystemType<\/key>\s*<string>apfs<\/string>/.test(plist);
      if (encrypted) return { encrypted: true, evidence: 'diskutil reports the volume as Encrypted' };
      if (locked) return { encrypted: false, evidence: 'diskutil reports an APFS volume that is not encrypted' };
      return { encrypted: null, evidence: 'diskutil did not state whether the volume is encrypted' };
    }
    if (platform === 'linux') {
