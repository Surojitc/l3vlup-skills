...
export const PUBLISHED_CLAIM_FIELDS = Object.freeze([
  'claimId', 'candidateId', 'rank', 'managerId', 'managerName', 'documentId', 'accession', 'filingDate', 'form',
 ...
 */
export function buildFeed({ claims = [], references = new Map(), dropped = [], stripped = [], ledger = {}, model, promptVersion, runId = null, coverage = [], selectionRejects = [] }) {
  const published = [];
  const refused = [];
  ...
  return {
    ...
    // Every document and every chunk, so a reader of the feed can see that it
    // is whole without trusting that the run would have said otherwise.
    coverage: [...coverage].sort((a, b) => String(a.documentId).localeCompare(String(b.documentId))),
    selectionRejects: [...selectionRejects].sort((a, b) => `${a.candidateId}${a.reason}`.localeCompare(`${b.candidateId}${b.reason}`)),
    cost: costLedger(ledger),
    quotedWordsByDocument: quotedByDocument,
  };
}

/** Costs, split by stage, from a ledger whose calls carry one. */
export function costLedger(ledger = {}) {
  const calls = (ledger.calls || []).map((c) => ({
    stage: c.stage ?? 'extraction', documentId: c.documentId, chunkId: c.chunkId, model: c.model,
    inputTokens: c.inputTokens, outputTokens: c.outputTokens, estimatedUsd: c.estimatedUsd, actualUsd: c.actualUsd,
  }));
  const byStage = {};
  for (const c of calls) {
    const row = byStage[c.stage] ?? (byStage[c.stage] = { calls: 0, inputTokens: 0, outputTokens: 0, actualUsd: 0 });
    row.calls += 1;
    row.inputTokens += c.inputTokens || 0;
    row.outputTokens += c.outputTokens || 0;
    row.actualUsd = Number((row.actualUsd + (c.actualUsd || 0)).toFixed(6));
  }
  return {
...
/** Whether every selected document processed every one of its chunks. */
export function coverageProblems(coverage, expectedDocuments = null) {
  const problems = [];
  if (!Array.isArray(coverage) || !coverage.length) return ['the run reported no coverage at all'];
  for (const c of coverage) {
    if (!c || typeof c !== 'object') { problems.push('a coverage row is not an object'); continue; }
    if (c.complete !== true) problems.push(`${c.documentId}: ${c.chunksProcessed} of ${c.chunksPlanned} chunk(s) processed${c.reason ? ` (${c.reason})` : ''}`);
    else if (c.chunksProcessed !== c.chunksPlanned) problems.push(`${c.documentId}: claims completeness but processed ${c.chunksProcessed} of ${c.chunksPlanned}`);
  }
  if (expectedDocuments !== null && coverage.length !== expectedDocuments) {
    problems.push(`${coverage.length} document(s) reported coverage where ${expectedDocuments} were selected`);
  }
  return problems;
}
...
/** The only reasons a failed run may name. Anything else becomes `unknown`. */
export const FAILURE_REASONS = Object.freeze([
  'truncated', 'refused', 'malformed_response', 'missing_usage', 'timeout',
  'rate_limited', 'no_credential', 'unknown_model', 'api_error', 'budget_exhausted',
  'fetch_refused', 'parse_failed', 'feed_invalid', 'incomplete_coverage', 'unknown',
]);

/**
 * What a failed run may hand back.
 *
 * A run that dies has already spent money, and until now it reported nothing:
 * the artefact upload only ran on success, so the runs whose cost you most
 * want to know were the ones that told you least.
 *
 * This is deliberately not the feed with the claims removed. It is built from
 * a closed list of fields, so there is no path by which a quotation, a chunk
 * of extracted text, a partial tool input or a claim could ride along. The
 * reason is snapped to a known vocabulary and the detail is our own sentence,
 * truncated, never the model's words and never the document's.
 */
export function buildFailureReport({
  reason, detail = '', ledger = {}, model = null, promptVersion = null, runId = null,
  documentsAttempted = 0, documentsSelected = 0, documentsFetched = 0, documentsStarted = 0,
  documentsCompleted = 0, failure = null, stage = null, coverage = [],
}) {
  return {
    feedVersion: FEED_VERSION,
    failed: true,
    note: 'A run that did not finish. Costs only: no claims, no source text, no quotations, no model output.',
    reason: FAILURE_REASONS.includes(reason) ? reason : 'unknown',
    detail: String(detail).slice(0, 200),
    model,
    promptVersion,
    runId,
    // Four counts, each an integer. Together they say how far the run got:
    // selected but never fetched is a network or SEC problem, fetched but
    // never started is parsing, started but never completed is the model.
    documentsSelected: Number(documentsSelected) || 0,
    documentsFetched: Number(documentsFetched) || 0,
    documentsStarted: Number(documentsStarted) || 0,
    documentsCompleted: Number(documentsCompleted) || 0,
    documentsAttempted,
    // The error that stopped the run, rebuilt field by field from a closed
    // list rather than copied. `reason` is one of our own classifications and
    // `detail` is the API's own message about the request, truncated: neither
    // is model output, and no other key survives the rebuild.
    failure: failure && typeof failure === 'object' ? {
      reason: FAILURE_REASONS.includes(failure.reason) ? failure.reason : 'unknown',
      detail: String(failure.detail ?? '').slice(0, 200),
      stage: ['extraction', 'consolidation', null].includes(failure.stage) ? failure.stage : null,
    } : null,
    stage: ['extraction', 'consolidation', null].includes(stage) ? stage : null,
    // Counts only. A coverage row names a document and two integers, which is
    // what tells you the run stopped early without telling you anything it read.
    coverage: (Array.isArray(coverage) ? coverage : []).map((c) => ({
      documentId: c.documentId, chunksPlanned: c.chunksPlanned, chunksProcessed: c.chunksProcessed, complete: c.complete === true,
    ...
    cost: {
      ...
      calls: (ledger.calls || []).map((c) => ({
        documentId: c.documentId, chunkId: c.chunkId, model: c.model,
        inputTokens: c.inputTokens, outputTokens: c.outputTokens,
        estimatedUsd: c.estimatedUsd, actualUsd: c.actualUsd,
      })),
    },
  };
}

/** Fields a failure report may carry. Anything else is a leak. */
export const FAILURE_REPORT_FIELDS = Object.freeze([
  'feedVersion', 'failed', 'note', 'reason', 'detail', 'model', 'promptVersion',
  'runId', 'documentsAttempted', 'documentsSelected', 'documentsFetched',
  'documentsStarted', 'documentsCompleted', 'failure', 'stage', 'coverage', 'cost',
]);

/** Read a failure report back and refuse it if it carries anything it may not. */
export function validateFailureReport(report) {
  const problems = [];
  if (!report || typeof report !== 'object') return ['the failure report is not an object'];
  if (report.failed !== true) problems.push('a failure report must say failed: true');
  for (const key of Object.keys(report)) {
    if (!FAILURE_REPORT_FIELDS.includes(key)) problems.push(`${key} is not a failure-report field`);
  }
  if (!FAILURE_REASONS.includes(report.reason)) problems.push(`${report.reason} is not a known failure reason`);
  if (String(report.detail ?? '').length > 200) problems.push('the detail is longer than 200 characters');

  // Nothing that could be a claim, a quotation or a passage, at any depth.
  const walk = (value, path) => {
    if (!value || typeof value !== 'object') return;
    for (const [k, v] of Object.entries(value)) {
      if (FORBIDDEN_PUBLISHED_FIELDS.includes(k)) problems.push(`${path}.${k} may never leave the runner`);
      if (['claims', 'proposals', 'refused', 'dropped', 'stripped', 'excerpt', 'paraphrase', 'evidenceExcerpt', 'quotedWordsByDocument'].includes(k)) {
        problems.push(`${path}.${k} has no place in a failure report`);
      }
      walk(v, `${path}.${k}`);
    }
  };
...
export function validateFeed(feed) {
  ...
  // A partial document is not a smaller feed, it is a wrong one: the claims
  // read identically and nothing on the page could say otherwise.
  problems.push(...coverageProblems(feed.coverage).map((p) => `incomplete coverage: ${p}`));
  // Six per document, counted from the feed rather than trusted from the run.
  const perDocument = {};
  for (const c of feed.claims) perDocument[c.documentId] = (perDocument[c.documentId] || 0) + 1;
  for (const [documentId, n] of Object.entries(perDocument)) {
    if (n > MAX_CLAIMS_PER_DOCUMENT) problems.push(`${documentId}: ${n} claims, over the ${MAX_CLAIMS_PER_DOCUMENT}-claim cap for one document`);
  }
  return problems;
}
