 ...
 *
 * Both entries were wrong when they were first written from memory, which is
 * the reason this comment exists. The identifiers are complete as published
 * and take no date suffix: `claude-haiku-4-5`, not
 * `claude-haiku-4-5`. And Sonnet 5 is $2/$10 per million tokens, not
 * the $3/$15 I had recorded, which was the previous generation's price.
 *
 * A wrong identifier fails loudly at the first call. A wrong price does not:
 * it silently mis-states every budget check, which is worse. So the figures
 * are dated, and `preflightModel` in lib/thesis-anthropic.mjs asks the
 * Models API to confirm the identifier before a single billable token is
 * spent.
 *
 * Verified against the Anthropic model table, cached 2026-06-24.
 */
export const MODEL_ALLOWLIST = Object.freeze({
  'claude-haiku-4-5': { inputPerMTok: 1.00, outputPerMTok: 5.00, contextTokens: 200_000, role: 'extraction', pricedOn: '2026-06-24' },
  'claude-sonnet-5': { inputPerMTok: 2.00, outputPerMTok: 10.00, contextTokens: 1_000_000, role: 'escalation on a failed verification', pricedOn: '2026-06-24' },
});

/** The one model the pilot is authorised to use. */
export const PILOT_MODEL = 'claude-haiku-4-5';
