...
import { chunkDocument, locateExcerpt } from './thesis-chunk.mjs';
import { stripToProposable } from './thesis-model.mjs';
import { resolveIssuerMention } from './security-master.mjs';
import { checkBudget, emptyCostLedger, LIMITS, MAX_CANDIDATES_PER_CHUNK, MAX_CLAIMS_PER_DOCUMENT, recordCall } from './thesis-cost.mjs';
import { filterAlreadyRejected } from './thesis-review.mjs';
import { candidateId, capList, dedupeCandidates, forConsolidation, resolveSelections } from './thesis-candidates.mjs';
...
export function proposalToClaim(raw, { chunk, document, manager, sourceText, taxonomy, aliases, model, promptVersion }) {
  ...
  // The offsets are computed here, from the excerpt, rather than taken from
  // the model. See locateExcerpt in thesis-chunk.mjs for why.
  const offsets = locateExcerpt(chunk, proposal.evidenceExcerpt);
  if (offsets.problem) return { ...drop(chunk, proposal, `evidence: ${offsets.problem}`), stripped };
...
export async function runDocument({
  ...
  // A parameter for the same reason `maxChunks` is one: the validation suites
  // drive a fixture of nine deliberately malformed proposals through one
  // chunk, and a cap applied there would hide what they exist to check. Every
  // caller that runs for real takes the default.
  ...
  for (const chunk of chunks) {
    ...
    // The per-chunk cap, held here rather than in the tool schema.
    //
    // A strict schema cannot carry `maxItems`, so the model is asked for at
    // most two in the tool description and held to it afterwards, on the
    // verified list. Anything past the cap is recorded as a drop with its own
    // reason, so overshoot is visible in the run rather than silent.
    const verified = [];
    ...
    const { kept: withinCap, discarded } = capList(verified, maxCandidatesPerChunk);
    for (const over of verified.slice(maxCandidatesPerChunk)) {
      droppedRows.push({ ...over, dropped: true, reason: `over the ${maxCandidatesPerChunk}-candidate cap for one chunk` });
    }
    if (discarded) notes.push(`${chunk.chunkId}: ${discarded} verified candidate(s) past the ${maxCandidatesPerChunk} cap`);
    for (const result of withinCap) kept.push({ result, chunk });
  }
