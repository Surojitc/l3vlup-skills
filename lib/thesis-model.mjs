...
export const NOT_PROPOSABLE = Object.freeze({
  evidenceStartOffset: 'the position of a quotation is found in the text, not counted by a model',
  evidenceEndOffset: 'the position of a quotation is found in the text, not counted by a model',
 ...
 */
export function fakeModel({ responses, select, id = 'fake-model-v1' }) {
  const calls = [];
  return {
    id,
    calls,
    async propose(request) {
      calls.push({ stage: 'extraction', chunkId: request.chunkId, documentId: request.documentId });
    ...
    /**
     * Stage B. The default ranks every candidate in the order it was given,
     * which keeps existing tests meaningful: they assert on what extraction
     * produced, and a selector that dropped things silently would hide that.
     * A test that cares about consolidation passes its own `select`.
     */
    async select(request) {
      calls.push({ stage: 'consolidation', documentId: request.documentId, candidates: request.candidates.length });
      if (typeof select === 'function') return select(request);
      return {
        selections: request.candidates.map((c, i) => ({ candidateId: c.candidateId, rank: i + 1 })),
...
export function unpricedModel(id = 'mystery-model-v9') {
  return {
    id,
    async propose() { throw new Error('this model should never have been called'); },
    async select() { throw new Error('this model should never have been called'); },
  };
}
