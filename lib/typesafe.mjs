 ...
 * throws the calibration away without telling anybody.
 */
...
export function emptyLedger({ budgetUsd = LIMITS.hardStopUsd } = {}) {
  return {
    model: MODEL,
 ...
 */
export function liveClient({ timeoutMs = 20_000, maxRetries = 2 } = {}) {
...
export async function ask(client, ledger, { state, questions }) {
  ...
  ledger.actualUsd += costUsd(inputTokens);
...
export function classify(err) {
  switch (err?.name) {
    ...
    default: return typeof err?.status === 'number' ? 'api_error' : 'unknown';
  }
}
